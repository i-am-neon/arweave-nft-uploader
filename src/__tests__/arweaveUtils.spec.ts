...
import { getArweavePriceForBytesInWinstons, connectToLocalArweave, generateTestKey, uploadDataToArweave, connectToArweave, getTxnURI, mintTestWinstonsToKey } from '../utils/arweaveUtils';
...
describe('arweaveUtils', () => {
    ...
    it('should save data to Arweave', async () => {
        ...
        const spy_arweave_createTransaction = jest.spyOn(arweave, 'createTransaction').mockReturnValueOnce(Promise.resolve(tx));
        const spy_transaction_addTag = jest.spyOn(Transaction.prototype, 'addTag');
        const spy_arweave_transactions_sign = jest.spyOn(arweave.transactions, 'sign').mockReturnValueOnce(Promise.resolve());
        const spy_arweave_transactions_getUploader = jest.spyOn(arweave.transactions, 'getUploader').mockReturnValueOnce(Promise.resolve(txUploader));
        const spy_transactionUploader_uploadChunk = jest.spyOn(TransactionUploader.prototype, 'uploadChunk').mockReturnValue(Promise.resolve());
    ...
    describe('ArLocal connection', () => {
        ...
        it('should create a new Arweave test wallet and load with money', async () => {
            const arweave = Arweave.init({});
            const spy_arweave_api_get = jest.spyOn(arweave.api, 'get').mockImplementation(() => Promise.resolve(mockedAxiosResponseTypeUnknown));
